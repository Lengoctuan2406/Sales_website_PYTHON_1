...
@app.route('/_admin/return_page')
def index():
    '''function index(): Load trang homepage dành cho user'''
...
def login():
    '''function login(): Load trang login dành cho user, dùng để đăng nhập'''
    msg = request.args.get('message', '')
    return render_template('_users/login.html', msg=msg)
@app.route('/_users/register')
def register():
    '''function register(): Load trang register dành cho user, dùng để đăng kí'''
    msg = request.args.get('message', '')
...
def page_category(product_type_id):
    '''function page_category(): Load trang dựa vào thể loại sản phẩm khi user chọn thể loại'''
    msg = request.args.get('message', '')
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    arrays_all_product = base_user_1_arrays_all_product(cursor)
    arrays_carts = base_user_1_arrays_carts(cursor)
...
def product_detail(product_id):
    '''function product_detail(): Load trang dựa sản phẩm mà user chọn vào'''
    msg = request.args.get('message', '')
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    arrays_all_product = base_user_1_arrays_all_product(cursor)
    arrays_carts = base_user_1_arrays_carts(cursor)
...
def page_product(page):
    '''function page_product(): Load trang chính bao gồm tất cả các sản phẩm hiện có'''
    page_convert = page
    msg = request.args.get('message', '')
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    arrays_all_product = base_user_1_arrays_all_product(cursor)
...
def page_carts():
    '''function page_carts(): Load trang giỏ hàng, bao gồm các sản phẩm mà khách hàng đã thêm vào'''
    msg = request.args.get('message', '')
    total = 0
    ...
    if(check_login()):
        ...
        total_all = 0
        list, total, order_product_all_id, order_all_quantity, total_all = get_product_cart_by_account(cursor)
...
def checkout():
    '''function checkout(): Load trang thanh toán, trang sẽ cho khách hàng thêm một số sửa đổi trước khi tạo đơn hàng thành công'''
    msg = request.args.get('message', '')
    total = 0
    order_product_all_id = 0
    order_all_quantity = 0
    ...
    if(check_login()):
        ...
        return render_template('_users/checkout.html', arrays_all_product=arrays_all_product, total_all=total_all, arrays_carts=arrays_carts, arrays_product_cart_by_account=list, total=round(total, 2), order_product_all_id=order_product_all_id, order_all_quantity=order_all_quantity, customer=customer, msg=msg)
    else:
        msg = 'You must be login first!'
    return redirect(url_for('login', message = msg))
@app.route('/_users/account_page', methods = ['POST', 'GET'])
def account_page():
    '''function account_page(): Trang chứa thông tin cá nhân của khách hàng khi đã đăng nhập vào'''
    msg = request.args.get('message', '')
    list = ()
    if(check_login()):
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
...
def account_reset_pass():
    '''function account_reset_pass(): Trang dùng để thay đổi mật khẩu của khách hàng khi đã đăng nhập'''
    msg = request.args.get('message', '')
    if(check_login()):
...
def account_order():
    '''function account_order(): Trang chứa các hóa đơn đang trong quá trình giao hàng của khách hàng'''
    msg = request.args.get('message', '')
    if(check_login()):
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
...
def account_purchase_history():
    '''function account_purchase_history(): Trang chứa lịch sử mua hàng của khách hàng'''
    msg = request.args.get('message', '')
    if(check_login()):
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
...
def checkout_check():
    ...
    if(check_login()):
        ...
        if request.method == 'POST' and 'orders' in request.form:
            ...
            order_status_id = 1
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute('INSERT INTO orders (account_id, order_product_all_id, order_all_quantity, order_total, order_name, order_address, order_phone, order_notes, order_status_id) VALUES \
                        (% s, % s, % s, % s, % s, % s, % s, % s, % s)',
                            (session['account_id'], order_product_all_id, order_all_quantity, order_total, order_name, order_address, order_phone, order_notes, order_status_id, ))
            mysql.connection.commit()
...
def login_check():
    '''function login_check(): Xử lý đăng nhập tài khoản'''
    msg = request.args.get('message', '')
...
def register_check():
    '''function register_check(): Xử lý tạo tài khoản mới'''
    msg = request.args.get('message', '')
